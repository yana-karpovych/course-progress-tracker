...
> API (`DELETE /lessons/:id`), frontend task (“Delete lesson button”), and acceptance checklist.
> **Required.**
...
During development, the **human** runs all checks — not the Agent.
...
- run `npm run dev` / `npm start` (leaves port 4000 busy → `EADDRINUSE`)
...
State the verification workflow once in the Superpowers session block at chat start.
Step prompts stay short: `executing-plans: Step X ONLY. Stop.`
